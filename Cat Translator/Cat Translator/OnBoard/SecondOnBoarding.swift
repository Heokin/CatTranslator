...
class SecondOnBoarding: UICollectionViewCell {
    ...
    lazy var label = Label(type: .boardingHeader, local.secondOnBoardingHeaderUp, local.secondOnBoardingHeaderDown)
    lazy var mainImage = ImageView(image: .boardingBig)
    
    var imageView: UIImageView = {
            let imageView = UIImageView(frame: .zero)
            imageView.image = UIImage(named: "backgroundOnBoard2")
